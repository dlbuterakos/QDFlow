...
class ThomasFermi:
    ...
    def calc_weight(self, u: NDArray[np.int_], v: NDArray[np.int_]) -> float:
        ...
        if num_isls == 1:
            if diff[0] == 1:
                weight = self.p_WKB[0] * self.fermi(E_v - E_u - mu_L) + self.p_WKB[
                    1
                ] * self.fermi(E_v - E_u - mu_R)
            elif diff[0] == -1:
                weight = self.p_WKB[0] * (
                    1 - self.fermi(E_u - E_v - mu_L)
    ...
    def sensor_from_island_charges(
    ...
    def calc_sensor(self) -> NDArray[np.float64]:
        ...
        """
        self.sensor_output = self.sensor_from_island_charges(
    ...
    def island_charges_from_charge_state(
    ...
    def sensor_from_charge_state(
        ...
        sensor : ndarray[float]
            ...
            at each sensor.
        """
        return self.sensor_from_island_charges(
            np.array(
