...
class Distribution(ABC, Generic[T]):
    ...
    @overload
    @abstractmethod
    def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]) -> NDArray: ...
    @abstractmethod
    def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]|None=None) -> T|NDArray:
        '''
...
class SumDistribution(Distribution[T], Generic[T]):
    ...
    def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]) -> NDArray: ...
    def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]|None=None) -> T|NDArray:
        d1 = (self.dist_1.draw(rng, size) if isinstance(self.dist_1, Distribution) else self.dist_1)
        d2 = (self.dist_2.draw(rng, size) if isinstance(self.dist_2, Distribution) else self.dist_2)
        if hasattr(d1, '__add__') or hasattr(d2, '__radd__'):
            return d1 + d2 # type: ignore[operator]
        else:
            raise ValueError("Addition not supported for %s and %s" % (repr(d1), repr(d2)))
...
class DifferenceDistribution(Distribution[T], Generic[T]):
    ...
    def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]|None=None) -> T|NDArray:
        ...
        if hasattr(d1, '__sub__') or hasattr(d2, '__rsub__'):
            ...
            raise ValueError("Subtraction not supported for %s and %s" % (repr(d1), repr(d2)))
...
class ProductDistribution(Distribution[T], Generic[T]):
    ...
    def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]|None=None) -> T|NDArray:
        ...
        if hasattr(d1, '__mul__') or hasattr(d2, '__rmul__'):
            ...
            raise ValueError("Multiplication not supported for %s and %s" % (repr(d1), repr(d2)))
...
class QuotientDistribution(Distribution[Any]):
    ...
    '''
    def __init__(self, dist_1:Distribution[Any]|Any, dist_2:Distribution[Any]|Any):
        self.dist_1 = dist_1
        self.dist_2 = dist_2
        if not (isinstance(dist_1, Distribution) or isinstance(dist_2, Distribution)):
            self.dist_1 = Delta(dist_1)
    ...
    @overload
    def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]) -> NDArray: ...
    def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]|None=None) -> Any|NDArray:
        d1 = (self.dist_1.draw(rng, size) if isinstance(self.dist_1, Distribution) else self.dist_1)
        d2 = (self.dist_2.draw(rng, size) if isinstance(self.dist_2, Distribution) else self.dist_2)
        if hasattr(d1, '__truediv__') or hasattr(d2, '__rtruediv__'):
            return d1 / d2 # type: ignore[operator]
        else:
            raise ValueError("Division not supported for %s and %s" % (repr(d1), repr(d2)))
...
class Normal(Distribution[float]):
    ...
    @overload
    def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]) -> NDArray[np.float64]: ...
    def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]|None=None) -> float|NDArray[np.float64]:        
...
class LogUniform(Distribution[float]):
    ...
    def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]) -> NDArray[np.float64]: ...
    def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]|None=None) -> float|NDArray[np.float64]:        
        return np.minimum(self.max, np.maximum(self.min,
...
class Binary(Distribution[T], Generic[T]):
    ...
    def __repr__(self) -> str:
        return "distribution.Binary(%s, %s, %s)" % (repr(self.p), repr(self.success), repr(self.fail))
...
class Discrete(Distribution[int]):
    ...
    @overload
    def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]) -> NDArray[np.int_]: ...
    def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]|None=None) -> int|NDArray[np.int_]:        
...
class CorrelatedDistribution(Distribution[NDArray[Any]], Generic[T]):
    ...
    @abstractmethod
    def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]|None=None) -> NDArray:
        '''
        Draws one or more samples from the distribution.
    ...
    class __DependentDistributionCore(Generic[U]):
        def __init__(self, dist:'CorrelatedDistribution[U]'):
            ...
            self.dep_dist = tuple([CorrelatedDistribution.DependentDistribution(self, i)
                               for i in range(self.dist.num_variables)])
        
        @overload
        def draw(self, index:int, rng:np.random.Generator, size:None) -> U: ...
        @overload
        def draw(self, index:int, rng:np.random.Generator, size:int|tuple[int, ...]) -> NDArray: ...
        def draw(self, index:int, rng:np.random.Generator, size:int|tuple[int, ...]|None) -> U|NDArray:        
            size_tup = (1,) if size is None else ((size,) if isinstance(size, int) else size)
            if self.should_redraw[index] or self.values is None or \
                        len(self.old_size)!=len(size_tup) or not np.all(self.old_size == np.array(size_tup)):
                if not self.should_redraw[index]:
                    warnings.warn(DependentDistributionWarning("draw() called on linked dependent distributions with differing size parameters."))
                elif not np.all(self.should_redraw):
                    warnings.warn(DependentDistributionWarning("draw() called twice on dependent distribution without drawing other correlated variables."))
                self.values = self.dist.draw(rng, size_tup)
                self.should_redraw = np.full(self.dist.num_variables, False, dtype=np.bool_)
                self.old_size = np.array(size_tup)
            self.should_redraw[index] = True
            size_slice:tuple[slice|int,...] = (*((slice(None),)*len(size_tup)),index)
            return self.values[size_slice] if size is not None else self.values[0, index]
        
        def dependent_distributions(self) -> tuple['CorrelatedDistribution.DependentDistribution[U]', ...]:
            return self.dep_dist
    

    class DependentDistribution(Distribution[U], Generic[U]):
        def __init__(self, core:'CorrelatedDistribution.__DependentDistributionCore[U]', index:int):
            self.__core = core
            self.__index = index
        ...
        @overload
        def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]) -> NDArray: ...
        def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]|None=None) -> U|NDArray:        
            return self.__core.draw(self.__index, rng, size)

        def __repr__(self) -> str:
            return "%s.dependent_distributions()[%s]" % (repr(self.__core.dist), repr(self.__index))
    ...
    def dependent_distributions(self) -> tuple['CorrelatedDistribution.DependentDistribution[T]', ...]:
        '''
...
class MatrixCorrelated(CorrelatedDistribution[T], Generic[T]):
    ...
    def __repr__(self) -> str:
        return "distribution.MatrixCorrelated(%s, %s)" % (repr(self.matrix), repr(self.distributions))
...
class SphericallyCorrelated(CorrelatedDistribution[float]):
    ...
    def draw(self, rng:np.random.Generator, size:int|tuple[int, ...]|None=None) -> NDArray:
        if size is None:
            r = self.radius if isinstance(self.radius, float) else self.radius.draw(rng, size=None)
            x = rng.normal(0, 1, size=self.n)
            x_norm = np.sqrt(np.sum(x**2))
            return r * x / x_norm
        else:
            r_arr = np.full(size, self.radius) if isinstance(self.radius, float) else self.radius.draw(rng, size=size)
    ...
    def __repr__(self) -> str:
        return "distribution.SphericallyCorrelated(%s, %s)" % (repr(self.n), repr(self.radius))
